...
| **UAT-INF-11** | Transient Socket Drop & Auto-Reconnect | *Simulate transient FastMCP socket drop (`deepagent-ansible-mcp:8000`), DB pool blip, and LLM 503 gateway retry during active conversation loop.* | **Failure Detection**: Supervisor Daemon instantly detects degraded endpoint (<10s) and isolates failing socket. **Auto-Recovery**: MultiServerMCPClient & DB pool execute exponential backoff retries; agent restores session context automatically without dropping thread history. | Resilient circuit-breaker & auto-reconnect logic restores full operational state; user session continues seamlessly. |
