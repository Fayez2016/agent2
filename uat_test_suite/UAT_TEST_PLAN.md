...
## 🧪 Comprehensive UAT Battery Matrix (15 Scenarios)
...
| **UAT-SEC-14** | **FastMCP Embedded Security Guard & Injection Defense** | *Submit malicious payload or prompt injection attempting catastrophic deletion (`rm -rf /`, `mkfs.ext4`, `chmod -R 777 /`) via raw command execution or prompt override.* | **Structural Encapsulation**: Transparently frames query in `<user_operational_directive>`. **Physical Gatekeeper**: Embedded Python `SecurityGuard` inside FastMCP server intercepts and physically blocks destructive patterns in <1ms without contacting AAP or SSH. **False-Positive Immunity**: Verifies legitimate SRE queries (`journalctl | grep`, `df -h`, `cat /var/log/messages`) pass freely with 0 false positives. | Zero reliance on LLM good behavior; deterministic Python code blocks destructive payload; full audit entry in PostgreSQL. |
| **UAT-OPS-15** | **Automated Stack Maintenance & Dependency Upgrades** | *Execute `system_updater.sh` to perform automated discovery of outdated host/container packages, visual diff table generation, dependency updates, clean rolling container restarts, and post-update health verification.* | **Automated Discovery**: Scans host pip dependencies, container runtimes, and podman services. **Explicit Upgrade Execution**: Shows line-by-line upgrade and verified package status (`langgraph 1.2.11`, `fastapi 0.141.1`, `mcp 1.29.1`). **Rolling Restart**: Restarts all stack microservices and verifies supervisor health probe returns `200 OK (Healthy)`. | General-purpose, audit-ready update lifecycle without downtime or manual dependency tracking. |
