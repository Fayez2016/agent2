...
| :--- | :--- | :--- |
| **3.1 Infrastructure Deployment Options** | Deploy the production stack via one of two supported enterprise topologies: | **Option A (RHEL Server / VM)**: Single Podman Pod (`deepagent-prod-pod`) via `offline_install.sh` or `deploy_from_quay.sh`.<br>*Hardened for rootless Podman*: Self-contained in-memory env, pre-baked proxy TLS (no host mount conflict), PostgreSQL socket routed to `/tmp`, IPv4 `127.0.0.1` binding.<br>**Option B (Kubernetes / OpenShift / VMware Tanzu)**: Production manifests reflecting the 7 microservices. |
